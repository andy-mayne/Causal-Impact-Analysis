...
if uploaded_file != None:
    ...
    st.markdown('''This graph shows the data that has been uploaded and the chosen target and covariate fields. You should check to ensure that the covariates follow the same patterns as the target (blue line), but is **not** impacted by the change you are trying to measure marked in grey. The correlation score of the current chosen metrics is '''+str(df2[targetcolumn][covariatecolumn]))
